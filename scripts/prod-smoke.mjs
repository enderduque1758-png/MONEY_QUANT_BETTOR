...
const required=['dist/index.html','dist/src/main.js','dist/src/analytics-v2.js','dist/src/market-options.js','dist/src/production-engine.js','dist/src/integration-core.js','api/[...path].js','api/health.js','api/sports.js','api/odds.js','api/state.js','api/settle.js','api/closing-line.js','api/preflight.js','server/store.js','server/settlement.js','server/model-policy.js'];
...
if(apiFiles.length>12){console.error(`Vercel Hobby limit exceeded: ${apiFiles.length} Serverless Functions (max 12)`);process.exit(1)}
...
console.log('Production smoke OK:',required.length,'required files +',apiFiles.length,'Vercel functions (Hobby max 12) + advanced analytics + forced integration + server imports');
